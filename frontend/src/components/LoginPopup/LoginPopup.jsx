...
const LoginPopup = ({setShowLogin}) => {
  ...
  return (
    <div className='login-popup absolute w-full h-full grid  z-10 '>
        <form className='login-popup-container place-self-center w-[max(23vw,330px)] flex-col gap-[25px] px-[30px] py-[25px] rounded-[8px] text-[14px] animate-[fadeIn_0.5s] bg-[white]' action="">
            ...
            <button className='border-[none] p-[10px] rounded-[4px] mt-2 text-[white] w-full bg-[tomato] text-[15px] cursor-pointer'>{currState==="Sign Up"?"Create Account":"Login here"}</button>
            <div className='login-popup-conditions flex gap-[8px] mt-[10px]'>
                <input type="checkbox" className='mt-[5px]' required />
                <p>Remember me </p>
            </div>
            {/* <div className='m-2'> */}
            {currState==="Login"?<p>Create New Account <span className='text-[tomato] font-medium cursor-pointer' onClick={()=>setCurrState("Sign Up")}>Click Here</span></p>:<p>Already have an Account? <span className='text-[tomato] font-medium cursor-pointer' onClick={()=>setCurrState("Login")}>Login Here</span></p>}
            {/* </div> */}
